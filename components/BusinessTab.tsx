...
const BusinessTab: React.FC = () => {
  ...
  const renderDashboard = () => (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-6 duration-700">
      ...
      <section className="bg-white/5 backdrop-blur-3xl border border-white/10 rounded-2xl p-5 shadow-2xl overflow-hidden relative group">
        ...
        <div className="flex justify-between items-start mb-6 relative z-10">
          <div>
            <h3 className="text-xl font-black text-white tracking-tighter uppercase italic">Earnings Overview</h3>
            <p className="text-[9px] text-white/40 font-black uppercase tracking-[0.4em] mt-1">Net revenue performance (7d)</p>
          </div>
          <div className="flex flex-col items-end">
            <span className="text-2xl font-black text-white tracking-tighter">$12,450.00</span>
            <div className="flex items-center space-x-1.5 text-green-400 mt-1">
              <TrendingUp className="w-3 h-3" />
              <span className="text-[9px] font-black tracking-widest">+18.5%</span>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#0c0c0c] text-white font-sans pb-32 animate-in fade-in duration-500 relative overflow-hidden">
      ...
      <div 
        className={`fixed top-[92px] bottom-[56px] left-0 w-[82%] max-w-[300px] bg-[#0c0c0c]/95 backdrop-blur-3xl border-r border-y border-white/10 z-[110] transform transition-transform duration-500 ease-out shadow-[40px_0_100px_rgba(0,0,0,0.9)] rounded-r-2xl ${isCurtainOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        <div className="p-6 h-full flex flex-col">
          ...
          <div className="pt-6 border-t border-white/5 mt-auto">
             <button className="w-full flex items-center justify-center space-x-3 p-4 bg-white/5 text-white/40 rounded-2xl font-black text-[10px] uppercase tracking-[0.3em] border border-white/5 hover:bg-red-500/10 hover:text-red-500 hover:border-red-500/10 transition-all active:scale-95">
      ...
      <div className="fixed bottom-24 right-6 z-40">
        <button className="w-14 h-14 bg-gradient-to-tr from-pink-600 to-rose-500 text-white rounded-2xl flex items-center justify-center shadow-[0_10px_30px_rgba(236,72,153,0.3)] active:scale-90 transition-all border border-white/20">
...
const DrawerItem = ({ icon: Icon, label, active, onClick }: any) => (
  <button 
    onClick={onClick}
    className={`w-full flex items-center justify-between p-3.5 rounded-2xl transition-all duration-300 group ${active ? 'bg-pink-500 text-white shadow-[0_10px_20px_rgba(236,72,153,0.25)] scale-[1.02]' : 'text-white/40 hover:text-white hover:bg-white/5'}`}
  >
    <div className="flex items-center space-x-4">
...
const DashboardStatSmall = ({ label, value, change, isPositive, icon: Icon, color }: any) => (
  <div className="bg-white/5 backdrop-blur-xl p-4 rounded-2xl border border-white/10 shadow-2xl group hover:border-pink-500/30 transition-all">
    <div className="flex justify-between items-start mb-4">
      <div className={`p-2.5 bg-white/5 rounded-xl ${color} shadow-inner`}>
        <Icon className="w-4 h-4" />
      </div>
      <div className={`px-2 py-0.5 rounded-lg text-[7px] font-black uppercase tracking-widest ${isPositive ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
        {change}
      </div>
    </div>
