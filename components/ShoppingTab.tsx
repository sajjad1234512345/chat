...
const ShoppingTab: React.FC = () => {
  ...
  return (
    <div className="min-h-screen pb-24 animate-in fade-in duration-500">
      {/* Search Header */}
      <div className="px-4 py-0.5 bg-[#0c0c0c] sticky top-0 z-20 flex items-center space-x-3">
        <div className="relative group flex-grow">
          ...
          <input 
            ...
            className="w-full bg-white/5 border border-white/5 rounded-2xl py-1 pl-12 pr-4 text-sm font-medium text-white shadow-sm focus:ring-1 focus:ring-pink-500/20 transition-all outline-none"
          />
        </div>
        <button 
          onClick={() => setView('my-store')}
          className="w-7 h-7 bg-white/5 border border-white/5 rounded-2xl flex items-center justify-center text-white/50 active:scale-95 transition-all hover:bg-white/10 hover:text-white"
      ...
      <div className="px-4 mt-2 mb-2">
        <div className="relative h-36 rounded-[2.5rem] overflow-hidden group cursor-pointer border border-white/10" onClick={() => setView('ar')}>
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex flex-col justify-end p-6">
            <div className="flex items-center space-x-2 bg-pink-500 w-fit px-3 py-1 rounded-full text-[10px] font-black text-white uppercase tracking-widest mb-2 -mt-[1px] -mb-[3px] -mr-[4px] -ml-[3px]">
      ...
      {/* Categories */}
      <div 
        className="flex space-x-3 overflow-x-auto scrollbar-hide mb-2"
        style={{ 
          marginLeft: '9px', 
          width: '357px', 
          height: '22.5px', 
      ...
      >
        {['All', 'Fashion', 'Beauty', 'Art'].map(cat => (
          <button 
            key={cat}
            onClick={() => setActiveCategory(cat)}
            className={`flex-shrink-0 px-3 py-0.5 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all ${activeCategory === cat ? 'bg-white text-black shadow-lg' : 'bg-white/5 text-white/40 border border-white/10'}`}
          >
            {cat}
          </button>
        ))}
