    ...
    <div className="min-h-screen bg-[#000000] text-white font-sans selection:bg-pink-500/30 pb-24 overflow-x-hidden">
      ...
      <div className="relative z-10 animate-in fade-in duration-700">
        ...
        <header className="flex items-center justify-between px-4 py-4 backdrop-blur-md bg-black/40 sticky top-0 z-50">
          ...
          <MoreHorizontal 
            className="w-7 h-7 stroke-[2.5] cursor-pointer active:scale-90 transition-transform" 
      ...
      {/* Instagram-style Settings Bottom Sheet */}
      <AnimatePresence>
        {isSettingsOpen && (
          <>
            {/* Backdrop */}
            ...
            <motion.div 
              ...
              className="fixed bottom-0 left-0 right-0 bg-[#1c1c1c] rounded-t-[24px] z-[101] max-h-[85vh] overflow-y-auto pb-10"
            >
              <div className="flex flex-col items-center">
                {/* Handle */}
                <div className="w-10 h-1 bg-white/20 rounded-full my-3" />
                
                <div className="w-full px-4">
                  <div className="flex items-center justify-between py-2 mb-4">
                    <h2 className="text-[16px] font-black tracking-tight w-full text-center">Settings</h2>
                  </div>

                  <div className="space-y-1">
                    <SettingItem icon={<Settings className="w-5 h-5" />} label="Settings and privacy" />
                    <SettingItem icon={<AtSign className="w-5 h-5" />} label="Threads" />
                    <SettingItem icon={<History className="w-5 h-5" />} label="Your activity" />
                    <SettingItem icon={<History className="w-5 h-5" />} label="Archive" />
                    ...
                    <SettingItem icon={<Ban className="w-5 h-5" />} label="Blocked" />
                    <SettingItem icon={<Info className="w-5 h-5" />} label="About" />
                    <div className="h-[1px] bg-white/5 my-2" />
                    <SettingItem icon={<LogOut className="w-5 h-5 text-red-500" />} label="Log out" textColor="text-red-500" />
                  </div>
                </div>
              </div>
            </motion.div>
...
const SettingItem: React.FC<{ 
  ...
  textColor?: string;
  onClick?: () => void;
}> = ({ icon, label, textColor = "text-white", onClick }) => (
  <button 
    onClick={onClick}
    className="w-full flex items-center space-x-4 py-3.5 px-2 hover:bg-white/5 active:bg-white/10 transition-colors rounded-xl"
  >
    <div className={textColor}>{icon}</div>
    <span className={`text-[15px] font-bold tracking-tight ${textColor}`}>{label}</span>
  </button>
);
