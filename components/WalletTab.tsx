    ...
    <div className="bg-transparent min-h-screen pb-32 animate-in fade-in duration-500">
      ...
      <header className="px-6 pt-8 pb-4 flex justify-between items-start">
        <div>
          ...
          <p className="text-sm text-white/40 font-medium mt-1">Manage your funds and cards</p>
        </div>
        <div className="flex space-x-3">
          <button className="w-10 h-10 rounded-full border border-white/10 bg-white/5 flex items-center justify-center shadow-sm text-white/60 active:scale-95 transition-all">
            <Settings className="w-5 h-5" />
          </button>
          <button className="w-10 h-10 rounded-2xl bg-pink-500/20 flex items-center justify-center shadow-sm text-pink-500 active:scale-95 transition-all border border-pink-500/20">
      ...
      <section className="px-6 mb-10">
        <div className="relative rounded-[2.5rem] p-8 text-white overflow-hidden shadow-2xl bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-500">
      ...
      <section className="mb-10">
        <div className="px-6 flex justify-between items-center mb-5">
          <h3 className="text-xl font-black text-white">My Cards</h3>
          <button className="bg-white/10 text-white px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider flex items-center space-x-1.5 active:scale-95 transition-all border border-white/5">
        ...
        <div className="flex space-x-4 overflow-x-auto px-6 scrollbar-hide py-2">
          {/* Card 1 - Blue */}
          <div className="min-w-[280px] h-48 rounded-[2.5rem] bg-gradient-to-br from-blue-600 to-indigo-700 p-7 text-white relative flex flex-col justify-between shadow-xl group cursor-pointer hover:scale-[1.02] transition-all">
          ...
          {/* Card 2 - Orange */}
          <div className="min-w-[280px] h-48 rounded-[2.5rem] bg-gradient-to-br from-orange-500 to-red-600 p-7 text-white relative flex flex-col justify-between shadow-xl group cursor-pointer hover:scale-[1.02] transition-all">
             <div className="flex justify-between items-start">
              <div className="w-12 h-8 bg-white/20 rounded-lg backdrop-blur-md flex items-center justify-center border border-white/10">
                 <div className="grid grid-cols-2 gap-0.5 px-2">
                   <div className="w-2 h-1 bg-white/40 rounded-sm" />
      ...
      <section className="px-6 mb-10">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-2xl font-black text-white tracking-tight">Weekly Insights</h3>
          <div className="flex space-x-4">
            <button 
              onClick={() => setActiveInsight('income')}
              className={`text-xs font-black uppercase tracking-widest px-3 py-1 rounded-lg transition-all ${activeInsight === 'income' ? 'text-pink-500 bg-white/10' : 'text-white/20'}`}
            >
              Income
            </button>
            <button 
              onClick={() => setActiveInsight('expense')}
              className={`text-xs font-black uppercase tracking-widest px-3 py-1 rounded-lg transition-all ${activeInsight === 'expense' ? 'text-pink-500 bg-white/10' : 'text-white/20'}`}
            >
              Expense
            </button>
          </div>
        ...
        <div className="bg-white/5 p-4 pt-10 rounded-[3rem] border border-white/10 shadow-sm flex flex-col h-64 relative overflow-hidden group backdrop-blur-md">
      ...
      <section className="px-6 mb-10">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-xl font-black text-white">Transactions</h3>
          <button className="text-pink-500 text-[10px] font-black uppercase tracking-widest">See All</button>
        </div>

        <div className="space-y-4">
          {MOCK_TRANSACTIONS.map((tx) => (
            <div key={tx.id} className="bg-white/5 backdrop-blur-md p-5 rounded-[2rem] border border-white/10 flex items-center justify-between shadow-sm active:scale-[0.98] transition-all cursor-pointer">
              <div className="flex items-center space-x-4">
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${tx.type === 'outgoing' ? 'bg-red-500/20 text-red-400' : 'bg-green-500/20 text-green-400'}`}>
              ...
              <div className={`text-sm font-black ${tx.type === 'outgoing' ? 'text-white' : 'text-green-400'}`}>
